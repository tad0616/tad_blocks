...
function setting_form($bid)
{
    ...
    foreach ($tags as $tag) {
        ...
        if ($start !== false) {
            ...
            $help = in_array($tag, $show_help) ? '若未上傳檔案，系統會自動根據下方設定產生對應圖片' : '';
        } else {
            $$selected_tag = '';
        }
...
<form action="ajax.php" method="post" enctype="multipart/form-data">
    ...
    <select name="tag" id="tag">
        ...
        <option value="hide" $selected_hide>[hide]隱藏標題</option>
        <option value="pic" $selected_pic>[pic]產生圖片取代標題</option>
        <option value="img" $selected_img>[img]產生圖片取代標題，並套用標題格式</option>
        <option value="icon" $selected_icon>[icon]文字標題左邊放上小圖示</option>
        <option value="link" $selected_link>[link]標題加上連結</option>
    </select>
    <input type="text" name="link_url" id="link"  placeholder="http://" $display_link>
    <input type="file" name="tag2" id="file" placeholder="請上傳圖片" $display_file>
