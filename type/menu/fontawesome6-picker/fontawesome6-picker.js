(function($) {
    ...
    // 從文字檔讀取圖示名稱並初始化選擇器
    function initializeIconPicker(basePath = '', options = {}) {
        // 預設選項
        ...
        $.get(basePath + 'free.txt', function(freeData) {
            ...
            $.get(basePath + 'brands.txt', function(brandData) {
                ...
                console.error('無法載入品牌圖示 (路徑: ' + basePath + 'brands.txt):', error);
            });
        }).fail(function(error) {
            console.error('無法載入免費圖示 (路徑: ' + basePath + 'free.txt):', error);
        });
    }
    ...
    function bindEvents() {
        ...
        // 為所有 iconPicker input 綁定 focus 事件
        $('.icon-picker-input').off('focus').on('focus', function() {
            $currentInput = $(this);
            $picker.show();
            $backdrop.show();
        });
    ...
    // 插件定義
    $.fn.iconPicker = function(basePath = '', options = {}) {
        // 標記這些輸入框
        this.addClass('icon-picker-input');

        // 如果選擇器還不存在，初始化它
        if (!$('.icon-picker-container').length) {
            initializeIconPicker(basePath, options);
        }
